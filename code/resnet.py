...
def _shortcut(input, residual):
    ...
    if stride_width > 1 or stride_height > 1 or stride_depth > 1 or not equal_channels:
        shortcut = Conv3D(filters=residual_shape[CHANNEL_AXIS],
                          kernel_size=(1, 1, 1),
                          strides=(stride_depth, stride_width, stride_height),
                          padding="valid",
...
class ResnetBuilder(object):
    @staticmethod
    def build(x, block_fn, repetitions, k_first, nfeat,  **kwarg):
        ...
        block_fn = _get_block(globals()[block_fn])

        input = x
        conv1 = _conv_bn_relu(filters=64, kernel_size=(k_first, k_first, k_first), strides=(2, 2, 2))(input)
        ...
        pool2 = AveragePooling3D(pool_size=(block_shape[DEP_AXIS], block_shape[ROW_AXIS], block_shape[COL_AXIS]),
                                 strides=(1, 1, 1))(block)
